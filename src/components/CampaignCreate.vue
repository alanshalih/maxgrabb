...
<div style="overflow-y:auto;overflow-x:hidden;max-height:calc(100vh - 210px);">
      ...
      <v-textarea auto-grow label="Pesan" v-model="new_data.message"></v-textarea>
    ...
    <img style="width : 100%" class="mt-3" v-if="image_preview" :src="image_preview" alt=""></div>
     <v-btn color="primary" @click="addNewCampaign()" class="mt-3">
     ...
     console.log('Error: ', error);
        ...
        FetchContact()
        {
            ...
            this.new_data.lists.forEach(list=>{
                  var contacts =   ipc.sendSync('contacts',{id : this.tab.id, params : {id : list.id}});
        ...
        addNewCampaign()
        {
             ...
             ipc.sendSync('add-campaign',{id : this.tab.id, data : this.new_data});
              this.$emit('routeTo',{tab_id : this.tab.id, route : 'campaign-detail', params : {id : this.new_data.id}});
        }
    }
}
</script>